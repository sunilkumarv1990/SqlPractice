...
-- What query will you write to find the number of customers who placed an order from more than one store?
...
where customer_id in ( select customer_id from sales_fact
group by customer_id
...
-- My Query
select count(order_id),city from sales_fact
inner join branch_details using (store_id)
group by city
limit 1;
...
-- My Query
select sum(p.sale_price), bd.area from product_info p
inner join order_product_mapping opm on 
p.product_id = opm.product_id
inner join sales_fact sf on 
sf.order_id = opm.order_id
inner join branch_details bd on 
...
-- My Query
select count(customer_id) , state from customer_info
inner join sales_fact using (customer_id)
inner join branch_details using (store_id)
group by state
